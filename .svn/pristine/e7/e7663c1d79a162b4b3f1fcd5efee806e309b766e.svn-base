		...
		function render_field( $field_params, $field_slug, $current_values, $in_the_bottom = false ) {
			...
			if ( $field_params['type'] == 'text' ) {
				...
				if ( function_exists( 'pmai_render_field' ) ) {
					echo wp_kses( pmai_render_field( $fieldData, ( ! empty( $current_values ) ) ? $current_values : array() ), self::ALLOWED_HTML );
				}
			} else if ( $field_params['type'] == 'title' ) {
				?>
                <h4 class="wpallimport-add-on-options-title"><?php echo esc_html( $field_params['name'] ); ?><?php if ( ! empty( $field_params['tooltip'] ) ): ?>
                    <a href="#help" class="wpallimport-help"
                       title="<?php echo esc_attr( $field_params['tooltip'] ); ?>"
                       style="position:relative; top: -1px;">?</a><?php endif; ?></h4>
				<?php

			} else if ( $field_params['type'] == 'plain_text' ) {
				if ( $field_params['is_html'] ):
					echo esc_html( $field_params['name'] );
				else:
					?>
		...
		function display_admin_notice() {
			...
			if ( ! get_option( sanitize_key( $this->slug ) . '_notice_ignore' ) ) {
                ...
                <div class="error notice is-dismissible wpallimport-dismissible" style="margin-top: 10px;"
                    ...
                    <p><?php echo wp_kses( sprintf( $notice_text, '?' . $this->slug . '_ignore=0' ), self::ALLOWED_HTML ); ?></p>
                </div>
